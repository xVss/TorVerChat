...
public class TVServer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            ThreadStanza rn2 = new ThreadStanza( connessione );
...
class ThreadConnessione implements Runnable {
    ...
    public static void SERVE(ThreadConnessione th) throws Exception {
        /* In questa modalita' il server attende istruzioni perservire poi il client dopo la fase di accesso */
        Command CMD = new Command("NaC");
        ...
        while ( CMD.type != Tipo.OUT ) {
            ...
            /* ================== JOINROOM ==================== */
                ...
                if( th.JOIN(room, psw) ){
                    ...
                    if( stanza != null ) {
                        stanza.add( th.sock );
                        th.outStream.writeObject( new Command("OK") );
                        System.out.println(th.sock.getInetAddress().toString() + " join in room " + room + " as " + th.user);
                    } else {
                        th.outStream.writeObject( new Command("ERR") );
                    }
                } else{
            ...
            /* ================== LEAVEROOM ==================== */
                ...
                if( stanza != null ) {
                    stanza.add( th.sock );
                    th.outStream.writeObject( new Command("OK") );
                    System.out.println(th.sock.getInetAddress().toString() + " leave in room " + room + " as " + th.user);
                } else {
                    th.outStream.writeObject( new Command("ERR") );
                }
            }
            ...
            /* ================== SEND ==================== */
                ...
                try {
                    ...
                    if( stanza != null ) {
                        ...
                        for(Socket s : stanza){
                            /*
                            BufferedWriter broadstream = new BufferedWriter( new OutputStreamWriter( s.getOutputStream() ) );
                            broadstream.write( th.user + ": " + txt + "\n"  );
                            ...
                            */
                            ...
                            Command mes = new Command("SEND");
                            mes.campi.put("MSG", txt);
                            mes.campi.put("FROM", th.user);
                            broadstream.writeObject( mes );
                            */
    ...
    public static void SENDMESSAGE(Socket s, String msg) throws Exception {
        ...
        DatagramPacket messaggio = new DatagramPacket(richiesta_b, richiesta_b.length, s.getInetAddress(), TVServer.porta+1);
